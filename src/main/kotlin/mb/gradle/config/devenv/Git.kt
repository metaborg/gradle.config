...
object Git {
    ...
    private fun internalExec(
        ...
        if (workingDir != null && !workingDir.exists()) return ExecResultImpl(
            "git",
            -255,
            ExecException("Cannot execute git command in directory $workingDir, directory does not exist")
        )
    ...
    fun getCurrentBranch(directory: File): String? {
        try {
            ...
            throw GradleException(
                "Cannot retrieve current branch name because no git repository was found at '$directory'",
                e
            )
        }.use { repo ->
